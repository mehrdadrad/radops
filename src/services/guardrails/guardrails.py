...
class GuardrailsOutput(BaseModel):
    ...
    is_safe: bool = Field(
        description="True if the input is safe and relevant, False if it is a jailbreak attempt or irrelevant."
    )
    reasoning: str = Field(description="Brief explanation of the safety decision.")


def guardrail(state: State, config: RunnableConfig):
    if not settings.agent.guardrail.enabled:
        return {
            "messages": [],
    ...
    llm = llm_factory(settings.agent.guardrail.llm_profile)
    ...
    guardrail_result = llm_structured.invoke(prompt)
