...
def create_history_deletion_tool(checkpointer):
    ...
    async def delete_conversation_history(user_id: str) -> dict:
        """Deletes the entire conversation history for the current user session. Call this tool when the user explicitly asks to forget, delete, or clear their conversation history."""
        if not checkpointer:
            return {"name": "history_tool_result", "content": "History is not being saved, so there is nothing to delete."}
        await checkpointer.adelete_thread(user_id)
        # Return a dictionary that the chatbot_node can inspect
        return {
            "name": "history_deleted", 
            "content": "Your conversation history has been successfully deleted. We are starting fresh."
        }
...
def create_history_retrieval_tool(checkpointer):
    ...
    async def get_conversation_history(user_id: str) -> str:
        """Retrieves the full conversation history for the current user session. After calling this tool, your ONLY response should be the verbatim output of this tool. Do not add any commentary or summarization. Just return the history as given."""
        if not checkpointer:
        ...
        formatted_history = "\n".join(
            f"{msg.type.capitalize()}: {msg.content}" for msg in recent_messages if isinstance(msg, (HumanMessage, AIMessage)) and not getattr(msg, 'tool_calls', None)
        )

        truncation_note = "\n(Note: Displaying the most recent part of a long conversation history.)" if is_truncated else ""

        return f"The user's conversation history has been retrieved. Your response should be ONLY the following text, without any additional commentary:\n\nHere is your conversation history:\n---\n{formatted_history}\n---{truncation_note}"
